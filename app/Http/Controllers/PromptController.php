...
class PromptController extends Controller
{
    ...
    try {
        ...
        if (isset($responseBody['candidates'][0]['content']['parts'][0]['text'])) {
            ...
            if (isset($matches[1])) {
                ...
                // Préparation des données pour l'enregistrement dans un fichier JSON
                $promptData = [
                ...
                $dataFilePath = storage_path('app/public/data/' . uniqid() . '.json');
                file_put_contents($dataFilePath, json_encode($promptData));
    ...
    public function lgeneratePrompt(Request $request)
{
    $prompt = Prompt::latest()->first();
        ...
        try {
            ...
            if (isset($responseBody['candidates'][0]['content']['parts'][0]['text'])) {
                ...
                if (isset($matches[1])) {
                    ...
                    $prompt->codeMermaid = $mermaidCode;
                    $prompt->save();
...
public function generatePrompt(Request $request)
{
    ...
    if ($prompt) {
        ...
        // Construction du second prompt
        $promptString = "Génère moi du code mermaid pour les description textuelle UML suivante $textInPrompt.
        Le code mermaid correcte doit être compris entre les caractères <!-- et -->. pour permettre l'affichage dans le frontend";
        ...
        try {
            ...
            if (isset($responseBody['candidates'][0]['content']['parts'][0]['text'])) {
                ...
                if (isset($matches[1])) {
                    ...
                    // Renvoyer le code Mermaid à la vue
                    return response()->json(['mermaidCode' => $mermaidCode]);
                } else {
                    Log::error('Mermaid code not found in response:', ['responseBody' => $responseBody]);
                    return response()->json(['error' => 'Le code Mermaid n\'a pas été trouvé dans la réponse.']);
                }
    ...
    {
        $prompt = Prompt2::findOrFail($id);

        return response()->json($prompt);
    }
}

// public function lgenerateDiagram(Request $request)
// {
//     // Validation des données du formulaire
//     $validatedData = $request->validate([
//         'description' => 'required|string',
...
//                 // Préparation des données pour la création de l'enregistrement
//                 $promptData = [
//                     'description' => $promptDescription,
//                     'codeMermaid' => $mermaidCode,
//                     'imagePath' => $imagePath,
